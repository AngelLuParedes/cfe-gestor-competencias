...
// ── Interfaces ────────────────────────────────────────────────────────────────
interface Conteo { insertados: number; actualizados: number; omitidos: number; }
interface ResultadoCarga { mensaje: string; insertados: number; omitidos: number; }
...
function fechaStr(val: unknown): string | null {
  if (val === null || val === undefined || val === '') return null;
  
  // Si Excel lo manda como número serial
  if (typeof val === 'number') {
    if (val < 10000) return null; // Destruye fechas cercanas a 1900/1899 automáticamente
  ...
  if (s.toUpperCase() === 'N/D' || s.toUpperCase() === 'N/A' || s === '0') return null;
  ...
  if (s.includes('/')) {
    ...
    if (partes.length === 3) {
      ...
      if (isNaN(anioNum) || anioNum < 1990) return null;
...
function diasDeCurso(horas: number | null): number {
...
function restarDiasHabiles(fechaISO: string, diasARestar: number): string {
...
// ── Parsers ───────────────────────────────────────────────────────────────────
function parsearEstandar(buffer: Buffer): Record<string, unknown>[] {
...
function parsearCCHL(buffer: Buffer): Record<string, unknown>[] {
  const wb = XLSX.read(buffer, { type: 'buffer', cellDates: false });
  if (!wb.SheetNames.length) throw new Error('El archivo no tiene hojas.');
...
function detectarFilaHeaders(sheet: XLSX.WorkSheet, maxFilas = 15): number {
  const ref = sheet['!ref'];
  if (!ref) return 8;
...
function esCCHL(buffer: Buffer): boolean {
  try {
    ...
    for (let row = 0; row < 10; row++) {
      for (let col = 0; col < 4; col++) {
        ...
        if (!cell) continue;
        const val = normalizar(String(cell.v ?? ''));
        if (val.includes('COMISION') && val.includes('ELECTR')) return true;
        if (val.includes('CCHL')) return true;
      }
    }
    return false;
  } catch { return false; }
...
function validar(filas: Record<string, unknown>[], cols: string[]): void {
  if (!filas.length) throw new Error('El archivo no contiene datos.');
  const presentes = Object.keys(filas[0]!);
  const faltan    = cols.filter(c => !presentes.includes(c));
  if (faltan.length) throw new Error(`Columnas faltantes: ${faltan.join(', ')}`);
}
...
async function upsertLote(
  ...
  if (!pool) throw new Error('Pool no inicializado. Revisa setPool() en index.ts.');
  ...
  for (let i = 0; i < filas.length; i += LOTE) {
    for (const fila of filas.slice(i, i + LOTE)) {
      try {
        const [h] = await pool.execute<ResultSetHeader>(sql, fila as any[]);
        if      (h.affectedRows === 1) insertados++;
        else if (h.affectedRows === 2) actualizados++;
      } catch (e: any) {
...
export const diagnostico = async (_req: Request, res: Response): Promise<void> => {
  const r: Record<string, unknown> = { pool_inicializado: !!pool };
  if (!pool) { r['solucion'] = 'Falta setPool(pool) en index.ts'; res.json(r); return; }

  try {
    const [dbRows] = await pool.execute<any[]>('SELECT DATABASE() AS db');
    r['base_de_datos'] = dbRows[0]?.db;

    const [tablas] = await pool.execute<any[]>(
      `SELECT table_name FROM information_schema.tables
       WHERE table_schema = DATABASE()
         AND table_name IN ('cursos','trabajadores','historial_capacitacion')`
    );
    r['tablas'] = tablas.map((t: any) => t.table_name);

    const cuentas: Record<string, unknown> = {};
    for (const t of ['cursos', 'trabajadores', 'historial_capacitacion']) {
      const [rows] = await pool.execute<any[]>(`SELECT COUNT(*) AS n FROM \`${t}\``);
      cuentas[t] = rows[0]?.n;
    }
    r['registros'] = cuentas;

    await pool.execute(
      `INSERT INTO cursos (ACTIVIDAD, NOMBRE_DEL_CURSO) VALUES (?,?)
       ON DUPLICATE KEY UPDATE NOMBRE_DEL_CURSO = VALUES(NOMBRE_DEL_CURSO)`,
      ['__DIAG_TEST__', '__DIAG_TEST__']
    );
...
export const cargarCompleto = async (req: Request, res: Response): Promise<void> => {
  try {
    if (!req.file) { res.status(400).json({ mensaje: 'No se recibió archivo.' }); return; }
    if (!pool)     { res.status(500).json({ mensaje: 'Pool no inicializado.' }); return; }
    ...
    filas = filas.filter(f => {
      ...
      if (isNaN(anio) || anio < 2021) return false;
    ...
    const cursosMap = new Map<string, unknown[]>();
    ...
    const trabMap = new Map<string, unknown[]>();
    for (const f of filas) {
      const rpe = str(f['RPE']);
    ...
    const resCursos = await upsertLote(
      regCursos,
      `INSERT INTO cursos
    ...
    const resTrab = await upsertLote(
      regTrab,
    ...
    const resHist = await upsertLote(
      historial,
...
export const cargarCursos = async (req: Request, res: Response): Promise<void> => {
  try {
    if (!req.file) { res.status(400).json({ mensaje: 'No se recibió archivo.' }); return; }
    if (!pool)     { res.status(500).json({ mensaje: 'Pool no inicializado.' }); return; }
    ...
    const r = await upsertLote(registros,
      `INSERT INTO cursos
         (SUBPROCESO,CLAVE_DEL_AREA,AREA,\`AÑO\`,ACTIVIDAD,
          CLAVE_ACTIVIDAD,CLAVE_CURSO,NOMBRE_DEL_CURSO,
          DURACION_REAL_HRS,clave_area_stps)
    ...
    res.json({ mensaje: 'Carga de cursos completada.', insertados: r.insertados + r.actualizados, omitidos: r.omitidos } as ResultadoCarga);
  } catch (err) {
...
export const cargarTrabajadores = async (req: Request, res: Response): Promise<void> => {
  try {
    if (!req.file) { res.status(400).json({ mensaje: 'No se recibió archivo.' }); return; }
    if (!pool)     { res.status(500).json({ mensaje: 'Pool no inicializado.' }); return; }
    const filas = parsearEstandar(req.file.buffer);
    ...
    res.json({ mensaje: 'Carga de trabajadores completada.', insertados: r.insertados + r.actualizados, omitidos: r.omitidos } as ResultadoCarga);
  } catch (err) {
    const m = err instanceof Error ? err.message : 'Error desconocido.';
    res.status(m.includes('Columnas') ? 400 : 500).json({ mensaje: m });
  }
...
export const cargarHistorial = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.json({ mensaje: 'Carga de historial completada.', insertados: r.insertados, omitidos: r.omitidos } as ResultadoCarga);
  } catch (err) {
    const m = err instanceof Error ? err.message : 'Error desconocido.';
    res.status(m.includes('Columnas') ? 400 : 500).json({ mensaje: m });
  }
...
export const cargarKardex = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const [rowsCursos]: any = await pool.query('SELECT ACTIVIDAD, CLAVE_CURSO FROM cursos');
    const mapaCursos = new Map<string, string>();
    for (const c of rowsCursos) {
      if (c.CLAVE_CURSO) mapaCursos.set(String(c.CLAVE_CURSO).trim().toUpperCase(), c.ACTIVIDAD);
    }

    const registrosHistorial: any[] = [];
    const cursosDetectados: any[] = [];
    ...
    if (cursosDetectados.length > 0) {
      ...
      await upsertLote(cursosDetectados,
        `INSERT INTO cursos (ACTIVIDAD, CLAVE_CURSO, NOMBRE_DEL_CURSO, DURACION_REAL_HRS)
         VALUES (?, ?, ?, ?)
         ON DUPLICATE KEY UPDATE
    ...
    const r = await upsertLote(registrosHistorial,
      `INSERT IGNORE INTO historial_capacitacion (rpe_trabajador,actividad_rel,fecha_inicio,fecha_termino,calificacion,estado_cchl) VALUES (?,?,?,?,?,?)`,
       [1062]
    );

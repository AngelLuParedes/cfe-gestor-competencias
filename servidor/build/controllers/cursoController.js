...
class CursoController {
    ...
    async buscar(req, res) {
        const { id } = req.params;
        const [curso] = await pool.query("SELECT * FROM cursos WHERE id_interno = ?", [id]);
        if (curso.length > 0) {
            return res.json(curso[0]);
        }
        res.status(404).json({ message: "No existe ese curso" });
    }
    async obtenerTrabajadoresPorCurso(req, res) {
        const { id } = req.params;
        try {
            const query = `
        SELECT 
    ...
    async actualizarHistorial(req, res) {
        ...
        try {
            await pool.query("UPDATE historial_capacitacion SET fecha_inicio = ?, fecha_termino = ?, horas_acreditadas = ?, calificacion = ? WHERE id_registro = ?", [
                fecha_inicio || null,
                fecha_termino || null,

...
async function obtenerDatosProcesados(mesFiltro = null) {
    ...
    const [historial] = await pool.query(queryHistorial);
    const [trabajadores] = await pool.query(queryTrabajadores);
    const [bateriasCursos] = await pool.query(queryBateriasCursos);
    let puente = [];
    try {
        const [puenteData] = await pool.query(queryPuente);
        puente = puenteData;
    }
    catch (e) {
    ...
    for (const t of trabajadores) {
        if (t.RPE)
            mapaTrabajadores.set(String(t.RPE).trim().toUpperCase(), t);
    }
    ...
    for (const registro of historial) {
        ...
        if (!rpeOriginal || !registro.id_registro)
            continue;
        ...
        if (!infoTrabajador || !infoTrabajador.PUESTO)
            continue;
...
class VigenciaController {
    async obtenerVigenciaCompleta(req, res) {
        try {
    ...
    async obtenerVigenciaPorMes(req, res) {
        try {
            const { mes } = req.params;
            const resultado = await obtenerDatosProcesados(parseInt(mes));
            res.json(resultado);
        }
        catch (error) {
            console.error("❌ Error:", error);
    ...
    async obtenerAlertasVigencia(req, res) {
        try {
            const resultado = await obtenerDatosProcesados();
            res.json(resultado);
        }
...
//# sourceMappingURL=Vigenciacontroller.js.map
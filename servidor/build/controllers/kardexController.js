...
class KardexController {
     ...
     */
    generarKardex = async (req, res) => {
        try {
            const { rpe_trabajador, nombre_trabajador, curp, puesto, ocupacion, cursos = [], } = req.body;
            ...
            for (const curso of cursos) {
                ...
                const nombreCorto = nombreCurso.length > 40
            ...
            const cursosAprobados = cursos.filter((c) => c.acreditado === "SI" || c.acreditado === "SÍ").length;
            // ✅ Solución al TS7006: Le agregamos el tipo ": number" a "sum" y ": any" a "c"
            const horasTotales = cursos.reduce((sum, c) => sum + (Number(c.horas) || 0), 0);
            const calificacionPromedio = cursos.length > 0
                ? (cursos.reduce((sum, c) => sum + (Number(c.calificacion) || 0), 0) /
                    cursos.length).toFixed(2)
                : "0";
            ...
            page.drawText(`Total de Cursos: ${totalCursos} | Acreditados: ${cursosAprobados} | Horas: ${horasTotales} | Promedio: ${calificacionPromedio}`, {
                x: 50,
                y: yPosition,
                size: 9,
                font: fuenteNormal,
            ...
            res.setHeader("Content-Disposition", `inline; filename="Kardex_${rpe_trabajador}.pdf"`);
     ...
     */
    generarKardexLote = async (req, res) => {
        try {
            ...
            res.setHeader("Content-Disposition", `inline; filename="Kardex_Lote_${new Date().getTime()}.pdf"`);
            res.send(Buffer.from(pdfBytes));
        }
        catch (error) {
...
//# sourceMappingURL=kardexController.js.map
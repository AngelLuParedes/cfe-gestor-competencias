...
class TrabajadoresController {
    ...
    async crear(req, res) {
        ...
        const { RPE, NOMBRE_COMPLETO, CURP, PUESTO, OCUPACION_ESPECIFICA, actividad, fecha_inicio, fecha_termino, calificacion, } = req.body;
        ...
        try {
            ...
            if (actividad) {
                const estado = parseFloat(calificacion) >= 80 ? "APROBADO" : "REPROBADO";
        ...
        try {
            ...
            if (error.errno === 1451) {
                res
                    .status(400)
                    .json({
                    message: "No se puede eliminar: El trabajador tiene historial de cursos.",
                });
            }
            else {
    ...
    async buscar(req, res) {
        const { rpe } = req.params;
        const [trabajador] = await pool.query("SELECT * FROM trabajadores WHERE RPE = ?", [rpe]);
...
//# sourceMappingURL=trabajadoresController.js.map
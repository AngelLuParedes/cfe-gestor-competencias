...
class ExcelLlenadorController {
    async llenarExcelConHistorial(req, res) {
        try {
            ...
            const [trabajadores] = await pool.query("SELECT * FROM trabajadores WHERE RPE = ?", [rpe_trabajador]);
            ...
            const [cursosBateria] = await pool.query(`SELECT bc.clave_curso, c.NOMBRE_DEL_CURSO 
         ...
         GROUP BY bc.clave_curso`, [bateriaFiltro, bateriaFiltro]);
            ...
            const [historial] = await pool.query("SELECT actividad_rel, fecha_termino, calificacion, estado_cchl FROM historial_capacitacion WHERE rpe_trabajador = ? AND fecha_termino IS NOT NULL", [rpe_trabajador]);
            const [puente] = await pool.query("SELECT ACTIVIDAD, CLAVE_CURSO FROM cursos");
    ...
    async llenarExcelMultiple(req, res) {
...
//# sourceMappingURL=excel-llenador.js.map
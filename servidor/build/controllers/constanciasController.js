...
const RUTA_PLANTILLA_PDF = path.join("C:/Users/pcAng/Downloads/cfe proyecto - copia/servidor/plantillas/DC3 MACHOTE Editableprueba.pdf");
class ConstanciasController {
    // ====================================================================
    // HELPERS (COMPARTIDOS Y DE WORD)
    // ====================================================================
    separarCaracteres(texto, prefijo) {
        const resultado = {};
    ...
    desglosarNombre(nombreCompleto) {
        if (!nombreCompleto)
    ...
    // ====================================================================
    escribirEnCasillas = (pagina, texto, xInicial, y, espaciado, fuente, size) => {
    ...
    inyectarDatosEnPDF = async (pagina, fuenteNormal, fuenteNegrita, trabajador, firmas) => {
        ...
        if (trabajador.clave_ocupacion) {
            const [rows] = await pool.query("SELECT ocupacion_especifica FROM ocupaciones WHERE clave = ?", [trabajador.clave_ocupacion]);
            if (rows.length > 0)
                textoOcupacion = rows[0].ocupacion_especifica;
        }
        ...
        if (trabajador.clave_area_stps && /^\d+$/.test(String(trabajador.clave_area_stps).trim())) {
            const [rows] = await pool.query("SELECT nombre_area FROM areas_tematicas WHERE clave = ?", [trabajador.clave_area_stps]);
            if (rows.length > 0)
        ...
        this.escribirEnCasillas(pagina, trabajador.CURP || "", 32, height - 175, 17.5, fuenteNegrita, 10);
        // =========================================================
        ...
        const dibujarFirma = (texto, centroX) => {
            if (!texto)
                return;
    ...
    crear = async (req, res) => {
        const { rpe_trabajador, clave_curso, puesto_en_constancia, clave_ocupacion, clave_area_stps, } = req.body;
        const datosRegistro = {
            rpe_trabajador,
            clave_curso,
        ...
        try {
            const [existe] = await pool.query("SELECT id_constancia FROM constancias_generadas WHERE rpe_trabajador = ? AND clave_curso = ?", [rpe_trabajador, clave_curso]);
            if (existe.length > 0) {
                await pool.query("UPDATE constancias_generadas SET ? WHERE id_constancia = ?", [datosRegistro, existe[0].id_constancia]);
    ...
    borrar = async (req, res) => {
        const { id } = req.params;
        await pool.query("DELETE FROM constancias_generadas WHERE id_constancia = ?", [id]);
    ...
    resetearEstado = async (req, res) => {
        const { id_registro } = req.params;
        try {
            // 1. Buscamos el registro usando "actividad_rel"
            const [hc] = await pool.query("SELECT rpe_trabajador, actividad_rel FROM historial_capacitacion WHERE id_registro = ?", [id_registro]);
            if (!hc.length) {
                res.status(404).json({ message: "Registro no encontrado" });
                return;
            }
            // 2. Borramos la constancia usando "actividad_rel"
            await pool.query("DELETE FROM constancias_generadas WHERE rpe_trabajador = ? AND actividad_rel = ?", [hc[0].rpe_trabajador, hc[0].actividad_rel]);
            // 3. Reseteamos el estado a "NO GENERADA"
            await pool.query("UPDATE historial_capacitacion SET estado_cchl = 'NO GENERADA' WHERE id_registro = ?", [id_registro]);
    ...
    generarPdfIndividual = async (req, res) => {
        try {
            const { rpe_trabajador, actividad_rel, fecha_inicio, id_registro, firmas = { instructor: "", patron: "", representante: "" }, } = req.body;
          ...
          WHERE t.RPE = ? AND c.ACTIVIDAD = ? ORDER BY hc.fecha_inicio DESC LIMIT 1`;
            ...
            await this.inyectarDatosEnPDF(pagina, fuenteNormal, fuenteNegrita, datos[0], firmas);
            const pdfBytes = await pdfDoc.save();
            res.setHeader("Content-Type", "application/pdf");
            res.setHeader("Content-Disposition", `inline; filename="DC3_${rpe_trabajador}.pdf"`);
    ...
    generarPdfLote = async (req, res) => {
        try {
            const { rpes_trabajadores, actividad_rel, firmas = { instructor: "", patron: "", representante: "" }, } = req.body;
      ...
      `;
            ...
            res.setHeader("Content-Type", "application/pdf");
            res.setHeader("Content-Disposition", `inline; filename="DC3_LOTE_${actividad_rel}.pdf"`);
            res.send(Buffer.from(pdfBytes));
        }
        catch (error) {
    ...
    // ====================================================================
    generarDocumento = async (req, res) => {
        const { rpe_trabajador, clave_curso, puesto_en_constancia, clave_ocupacion, clave_area_stps, } = req.body;
        try {
            const qTrabajador = "SELECT * FROM trabajadores WHERE RPE = ?";
            const [trab] = await pool.query(qTrabajador, [rpe_trabajador]);
            const qCurso = "SELECT * FROM cursos WHERE CLAVE_CURSO = ?";
            const [curso] = await pool.query(qCurso, [clave_curso]);
            const qFechas = "SELECT fecha_inicio, fecha_termino FROM historial_capacitacion WHERE rpe_trabajador = ? AND clave_curso_rel = ?";
            const [fechas] = await pool.query(qFechas, [
                rpe_trabajador,
                clave_curso,
            ]);
            let textoOcupacion = "";
            if (clave_ocupacion) {
                const qOcup = "SELECT ocupacion_especifica FROM ocupaciones WHERE clave = ?";
                const [ocupRows] = await pool.query(qOcup, [clave_ocupacion]);
                if (ocupRows.length > 0)
                    textoOcupacion = ocupRows[0].ocupacion_especifica;
                else
                    textoOcupacion = clave_ocupacion;
            }
            let textoArea = "";
            if (clave_area_stps) {
                const qArea = "SELECT nombre_area FROM areas_tematicas WHERE clave = ?";
                const [areaRows] = await pool.query(qArea, [clave_area_stps]);
                if (areaRows.length > 0)
                    textoArea = `${clave_area_stps} - ${areaRows[0].nombre_area}`;
                else
                    textoArea = clave_area_stps;
            }
            if (!trab[0] || !curso[0]) {
            ...
            const rutaPlantillaWord = path.resolve(__dirname, "../../plantillas/DC3 MACHOTE Editableprueba.docx");
            ...
            res.setHeader("Content-Disposition", `attachment; filename=DC3_${rpe_trabajador}.docx`);
    ...
    // ====================================================================
    generarPdfManual = async (req, res) => {
        try {
            ...
            const pdfBytes = await pdfDoc.save();
            res.setHeader("Content-Type", "application/pdf");
            res.setHeader("Content-Disposition", `inline; filename="DC3_MANUAL_${trabajador.rpe || "NUEVO"}.pdf"`);
            res.send(Buffer.from(pdfBytes));
        }
        catch (error) {
        ...
        try {
            ...
            for (const archivo of archivos) {
                try {
                    ...
                    console.warn("[Merge] Saltando archivo inválido:", archivo.originalname);
                }
            }
            ...
            const pdfBytes = await pdfMaestro.save();
            res.setHeader("Content-Type", "application/pdf");
            res.setHeader("Content-Disposition", `inline; filename="DC3_LOTE_MANUAL.pdf"`);
            res.send(Buffer.from(pdfBytes));
        }
        catch (error) {
...
//# sourceMappingURL=constanciasController.js.map
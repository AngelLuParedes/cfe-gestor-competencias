...
class AuthController {
    async register(req, res) {
      ...
      `;
            const [existente] = await pool.query(verificarExistente, [email, nombre_usuario]);
    ...
    async login(req, res) {
        ...
        WHERE email = ?
      `;
            const [usuarios] = await pool.query(consultaUsuario, [email]);
            ...
            await pool.query(`INSERT INTO sesiones (id_usuario, token, ip_address, user_agent, fecha_expiracion) 
         VALUES (?, ?, ?, ?, ?)`, [
                usuario.id_usuario,
                token,
                req.ip,
    ...
    async cambiarPassword(req, res) {
        ...
        try {
            const decoded = jwt.verify(token, JWT_SECRET);
            const [usuarios] = await pool.query('SELECT password_hash FROM usuarios WHERE id_usuario = ?', [decoded.id_usuario]);
            if (usuarios.length === 0) {
            ...
            await pool.query('UPDATE usuarios SET password_hash = ? WHERE id_usuario = ?', [nuevoHash, decoded.id_usuario]);
            await pool.query('UPDATE sesiones SET activa = FALSE WHERE id_usuario = ? AND token != ?', [decoded.id_usuario, token]);
    ...
    async forgotPassword(req, res) {
        ...
        try {
            const [usuarios] = await pool.query('SELECT id_usuario, email FROM usuarios WHERE email = ? AND activo = 1', [identificador]);
            if (usuarios.length === 0) {
            ...
            await pool.query('UPDATE usuarios SET password_hash = ?, fecha_modificacion = NOW() WHERE id_usuario = ?', [nuevoHash, usuario.id_usuario]);
            await pool.query('UPDATE sesiones SET activa = FALSE WHERE id_usuario = ?', [usuario.id_usuario]);
    ...
    async actualizarPerfil(req, res) {
        ...
        try {
            ...
            if (nombre_usuario)
                datosActualizar.nombre_usuario = nombre_usuario;
            if (Object.keys(datosActualizar).length === 0) {
                return res.status(400).json({ message: "No hay datos para actualizar" });
            }
            await pool.query('UPDATE usuarios SET ? WHERE id_usuario = ?', [datosActualizar, decoded.id_usuario]);
            const [usuarioActualizado] = await pool.query('SELECT id_usuario, nombre_usuario, email, nombre_completo, rol FROM usuarios WHERE id_usuario = ?', [decoded.id_usuario]);
            res.json({
